...
int back_cnt = 0;            //撤销操作的次数

struct Channel init_channel1(struct Channel channel1[]) {            //初始化通道

	for (int i = 1; i < 6; i++) {
		printf("这是%d号通道，请输入所摆放商品的名称（不在此通道摆放请输入END，如想要撤销上步摆放操作请直接输入back,最多三次）：\n", i);
...
void Trade(struct Channel channel[]) {                   //购买函数（包括找零）
	...
	int i = 0;
	while (1) {
		printf("请输入你要购买的商品名，所在通道和数量（请用空格隔开，结束选择请直接输入END ,如想撤销上步购买请直接输入back，最多三次）：\n");
